...
def bellman_ford(nodes, edges, source):
    ...
    for v in range(v_1):
        for key in edges:
            for y, _l in edges[key].items():
                new_path_len = path_len[key] + _l
                if (new_path_len < path_len[y]):
                    prev_nodes[y] = key
                    path_len[y] = new_path_len
...
1. graphs must be bi-directional, and edges must be without any weights;
2. here, it can find the nodes that are connected;
3. time complexity of Depth First Search algorithm is O( N + E )
